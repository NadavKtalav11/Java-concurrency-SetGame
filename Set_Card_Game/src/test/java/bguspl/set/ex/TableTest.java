...
class TableTest {
    ...
    // test that placing a card in slot increases the number of card on the table
    @Test
    void placeCard() {
        // checks that at the beginning the table have no cards on.
        ...
        assertEquals(8, slotToCard[2]);

        table.placeCard(8,0);
        assertEquals(2,table.countCards());

        table.placeCard(8,1);
        assertEquals(3,table.countCards());

        table.placeCard(8,3);
        assertEquals(4,table.countCards());
    }

    // tests that removing a card from the table decreases the amount of card on the table
    @Test
    void removeCard() {
        assertEquals(null, slotToCard[0]);
        assertEquals(0,table.countCards());
